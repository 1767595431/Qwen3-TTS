...
def load_gtcrn_model(checkpoint_path: str, device: str = "cpu"):
    """加载 GTCRN 模型"""
    model = GTCRN().eval()
    if os.path.isfile(checkpoint_path):
        ...
        if isinstance(state, dict):
            if "model" in state:
                # 训练checkpoint格式：{"epoch": ..., "optimizer": ..., "model": {...}}
                model.load_state_dict(state["model"], strict=False)
            elif "state_dict" in state:
                # 标准checkpoint格式：{"state_dict": {...}}
...
def enhance_audio(input_path: str, output_path: str, checkpoint_path: str, device: str = None, chunk_sec: float = 10.0):
    """
    对音频进行降噪/增强，输出 16kHz 单声道 wav。
    
    Args:
        input_path: 输入音频路径
        output_path: 输出 wav 路径
        checkpoint_path: GTCRN 模型权重路径（.tar）
        device: cuda/cpu，默认自动选择
        chunk_sec: 长音频分块处理时长（秒），避免 OOM
    """
    if sf is None or librosa is None:
        raise RuntimeError("请安装 soundfile 和 librosa: pip install soundfile librosa")
    
    device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
    model = load_gtcrn_model(checkpoint_path, device)
    
    wav, sr = librosa.load(input_path, sr=None, mono=True)
    if sr != GTCRN_SR:
        wav = librosa.resample(wav, orig_sr=sr, target_sr=GTCRN_SR)
        sr = GTCRN_SR
    
    wav = np.asarray(wav, dtype=np.float32)
    window = torch.hann_window(GTCRN_WIN, device=device).pow(0.5)
    
    chunk_samples = int(chunk_sec * GTCRN_SR)
    out_chunks = []
    
    for start in range(0, len(wav), chunk_samples):
        end = min(start + chunk_samples, len(wav))
        chunk = wav[start:end]
        if len(chunk) < GTCRN_HOP * 2:
            continue
    ...
    if not out_chunks:
        sf.write(output_path, wav, GTCRN_SR, format="WAV")
        return
    
    out_wav = np.concatenate(out_chunks, axis=0)
    sf.write(output_path, out_wav.astype(np.float32), GTCRN_SR, format="WAV")
