...
class XVectorExtractor(nn.Module):
    ...
    def sox_norm(self, audio):
        # Normalize audio to target dB level without sox
        audio = np.asarray(audio, dtype=np.float32)
        # Calculate RMS
        rms = np.sqrt(np.mean(audio ** 2))
        if rms > 0:
            # Convert target dB to linear scale
            target_linear = 10 ** (self.target_db_level / 20.0)
            # Normalize
            wav_norm = audio * (target_linear / rms)
            # Clip to prevent overflow
            wav_norm = np.clip(wav_norm, -1.0, 1.0)
        else:
            wav_norm = audio
        return wav_norm
